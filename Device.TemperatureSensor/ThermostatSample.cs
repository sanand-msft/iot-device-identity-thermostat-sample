...
namespace Device.TemperatureSensor
{
    ...
    public class ThermostatSample
    {
        ...
        public async Task PerformOperationsAsync(CancellationToken cancellationToken)
        {
            ...
            // -> Set handler to receive "reset" command, and send reset status as command response.
            ...
            _logger.LogDebug($"Set handler for \"ResetCommand\" command.");
            await _deviceClient.SetMethodHandlerAsync("reset", HandleResetCommand, _deviceClient, cancellationToken);
        ...
        private Task<MethodResponse> HandleResetCommand(MethodRequest request, object userContext)
        {
            try
            {
                Console.WriteLine("Command: Received direct method call to reset the thermostat.");
                ...
                object done = "completed";
                var report = new
                {
                    reset = done,
                };

                Console.WriteLine("Received direct method call to reset thermostat...");
                byte[] responsePayload = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(report));
                return Task.FromResult(new MethodResponse(responsePayload, (int)StatusCode.Completed));
                SendTemperature();
        ...
        private async Task SendTemperatureTelemetryAsync()
        {
            ...
            Console.WriteLine($"Telemetry: Sent - {telemetryName}:{_temperature}");
